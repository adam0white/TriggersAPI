...
superseded_reason: "Vite UI archived; backend UI already has metrics dashboard with different implementation"
---
...
The Metrics Pulse tiles and Delivery Analysis establish confidence in platform performance by making KPIs immediately visible. The sparklines and delta badges provide context (faster/slower than last run?), while the Run History panel enables power users to rapidly compare runs or replay successful scenarios. The DLQ visibility prevents surprises—failures are surfaced, not hidden. This story unblocks Story 7.6 (UX Pattern Implementation) and establishes the final data layer that makes the dashboard a credible showcase for edge-native event processing. Early completion allows concurrent work on Stories 7.6-7.7 (UX Patterns, Polish).
...
- [ ] Icon: Checkmark icon
- [ ] Delta badge: Change from last run (e.g., "-0.5% ↓" in amber for warning)
...
- [ ] Icon: Stopwatch icon
- [ ] Delta badge: Change from last run (e.g., "-15ms ↓" in green for faster)
...
- [ ] Delta badge: Current vs last measurement (e.g., "-1 event ↓")
...
- [ ] Hover: Subtle elevation increase (card-base → card-active shadow)
...
- ✅ 4 KPI metric tiles (Total Events, Success Rate, Avg Latency, Queue Depth) with real-time updates
- ✅ Delta badges showing change from previous run with directional indicators
- ✅ Sparklines for trend visualization (10 data points, responsive)
- ✅ State-based styling (normal, warning, critical) with color coding
- ✅ DLQ widget showing failed events with retry functionality
- ✅ Run history panel with chronological list (max 20 visible)
- ✅ Status badges (Success/Failed/Partial) and type indicators (Default/Debug/Bulk)
- ✅ Replay functionality with payload copying
- ✅ Filtering by status, type, and time range
- ✅ Search by run ID
- ✅ Bulk selection and CSV export
- ✅ Expandable run details showing payload and debug flags
- ✅ Real-time updates via RunContext integration
- ✅ Responsive layout (desktop 2x2 grid, mobile single column)
- ✅ WCAG 2.1 AA accessibility (aria-labels, keyboard navigation, live regions)
- ✅ All design tokens used from Story 7.1
...
- [x] Delta badges show directional indicators (↑↓→) with appropriate color coding
...
- Type indicators (📦🐛▶️) visually distinguish run types
...
- MetricsPanel ↔ RunContext.sessionMetrics
- HistoryPanel ↔ RunContext.runHistory
- Sparkline ↔ MetricTile
- DeltaBadge ↔ MetricsPanel
- DLQWidget ↔ MetricsPanel
