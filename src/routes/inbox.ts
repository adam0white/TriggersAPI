 ...
 */
export async function handleInboxQuery(request: Request, env: Env, correlationId: string): Promise<Response> {
	try {
		...
		for (const status of statusList) {
			if (!validStatuses.includes(status)) {
				return badRequest('INVALID_PARAMETER', correlationId, `Invalid status: ${status}. Must be one of: pending, delivered, failed`);
			}
		}
		...
		if (filterCount > 10) {
			return badRequest('TOO_MANY_FILTERS', correlationId, 'Maximum 10 filters allowed per query (DoS prevention)');
		}
		...
		if (!validSortFields.includes(sort)) {
			return badRequest('INVALID_PARAMETER', correlationId, `Invalid sort field: ${sort}. Must be one of: ${validSortFields.join(', ')}`);
		}

		// Validate sort order
		if (order !== 'asc' && order !== 'desc') {
			return badRequest('INVALID_PARAMETER', correlationId, `Invalid order: ${order}. Must be asc or desc`);
		}
		...
		if (from && isNaN(Date.parse(from))) {
			return badRequest('INVALID_PARAMETER', correlationId, `Invalid from timestamp: ${from}. Must be ISO-8601 format`);
		}

		if (to && isNaN(Date.parse(to))) {
			return badRequest('INVALID_PARAMETER', correlationId, `Invalid to timestamp: ${to}. Must be ISO-8601 format`);
		}

		// Validate created_date format (YYYY-MM-DD)
		if (createdDate && !/^\d{4}-\d{2}-\d{2}$/.test(createdDate)) {
			return badRequest('INVALID_PARAMETER', correlationId, `Invalid created_date: ${createdDate}. Must be YYYY-MM-DD format`);
		}
		...
		if (cursorParam) {
			...
			if (!cursorObj) {
				return badRequest('INVALID_CURSOR', correlationId, 'Invalid cursor format');
			}
		}
 ...
 */
export async function handleAckEvent(request: Request, env: Env, ctx: ExecutionContext, eventId: string): Promise<Response> {
	const correlationId = request.headers.get('x-correlation-id') || crypto.randomUUID();

	try {
		// Validate event_id format (UUID-like)
		if (!isValidUUID(eventId)) {
			return badRequest('INVALID_PARAMETER', correlationId, `Invalid event ID format: ${eventId}`);
		}
		...
		ctx.waitUntil(
			(async () => {
				try {
					...
					console.info(`[${correlationId}] Metrics updated for ack: event_id=${eventId}, status=${event.status}`);
				} catch (err) {
 ...
 */
export async function handleRetryEvent(request: Request, env: Env, ctx: ExecutionContext, eventId: string): Promise<Response> {
	const correlationId = request.headers.get('x-correlation-id') || crypto.randomUUID();
	...
	try {
		...
		if (event.status !== 'failed') {
			console.warn(`[${correlationId}] Cannot retry event with status: ${event.status} (must be 'failed')`);
			return new Response(
				JSON.stringify({
					error: {
						code: 'INVALID_STATE',
		...
		if (event.retry_count >= 3) {
			console.warn(`[${correlationId}] Event exceeded max retries (${event.retry_count}): ${eventId}`);
			return new Response(
				JSON.stringify({
					error: {
		...
		console.info(`[${correlationId}] Event queued for retry: ${eventId}, attempt: ${event.retry_count + 1}`);
