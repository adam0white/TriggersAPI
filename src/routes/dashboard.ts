...
// Source: src/ui/index.html (Story 2.6: with metrics panel)
...
<head>
  ...
  <style>
    ...
    * {
    ...
    h6 { font-size: var(--font-size-xs); font-weight: var(--font-weight-semibold); }
    ...
    .flow-line.active {
      stroke-width: 4;
      animation: flow-animation 1s linear infinite, glow 1.5s ease-in-out infinite;
    }

    @keyframes glow {
      0%, 100% { filter: drop-shadow(0 0 2px var(--color-primary)); }
      50% { filter: drop-shadow(0 0 6px var(--color-primary)); }
    }
    ...
    .dlq-arrow.active {
      opacity: 1;
      stroke-width: 4;
      animation: flow-animation 1s linear infinite, pulse-error 1.5s ease-in-out infinite;
    }
    ...
    .help-icon-btn:hover, .collapse-btn:hover {
      background: var(--color-bg-tertiary);
    }
...
<body>
  <div class="dashboard-container">
    <header>
      <div class="header-left">
        <h1>TriggersAPI</h1>
        <p style="color: var(--color-text-secondary); margin: 8px 0 0 0; font-size: 14px;">
  ...
  "source": "auth-service"
    ...
    <!-- Response Display - REMOVED: BUG FIX 3 - Using toasts instead of stale response panel
      ...
      <div class="debug-content" id="debug-content">
        ...
        <div class="debug-section">
          ...
          <div class="quick-actions" style="margin-bottom: 20px;">
            <button class="btn-submit" onclick="loadDemoData()" style="flex: 1;">
              Load Demo (5 events)
            </button>
            <button class="btn-secondary" onclick="showBatchGeneratorModal()" style="flex: 1;">
              Batch Generate
            </button>
            <button class="btn-secondary" onclick="generateRealisticScenario()" style="flex: 1;">
              Generate Scenario
            </button>
          </div>
        ...
        <div class="debug-section">
          ...
          <button class="scenario-btn" id="batch-submit-btn" style="display: none;">
            Run Batch Test
          </button>
        ...
        <div class="keyboard-shortcuts">
          ...
          <kbd>Alt+D</kbd> Toggle panel |
          <kbd>Alt+1</kbd> Validation Error |
          <kbd>Alt+2</kbd> Processing Error |
          <kbd>Alt+3</kbd> Queue Delay |
          <kbd>Alt+4</kbd> DLQ Routing
        </div>
      </div>
    </div>
    ...
    <section class="metrics-section" id="metricsSection">
      ...
      <div id="eventFlowSection" class="event-flow-section" style="display: block; margin-bottom: 32px;">
        ...
        <svg id="flowDiagram" viewBox="0 0 1200 400" class="flow-diagram" preserveAspectRatio="xMidYMid meet">
          ...
          <!-- Ingestion Stage -->
          <g id="stage-ingestion" class="stage-box" transform="translate(50, 80)" data-status="normal">
          ...
          <!-- Queue Stage -->
          <g id="stage-queue" class="stage-box" transform="translate(320, 80)" data-status="normal">
            <rect class="stage-rect" width="180" height="140" rx="8" fill="url(#normalGradient)"/>
          ...
          <!-- Processing Stage -->
          <g id="stage-processing" class="stage-box" transform="translate(590, 80)" data-status="normal">
            <rect class="stage-rect" width="180" height="140" rx="8" fill="url(#normalGradient)"/>
          ...
          <!-- Storage Stage -->
          <g id="stage-storage" class="stage-box" transform="translate(860, 80)" data-status="normal">
            <rect class="stage-rect" width="180" height="140" rx="8" fill="url(#normalGradient)"/>
          ...
          <g id="stage-dlq" class="stage-box dlq-stage" transform="translate(590, 280)" data-status="normal">
          ...
          <!-- Flow Arrows -->
          <!-- Ingestion → Queue -->
          <line x1="230" y1="150" x2="320" y2="150" class="flow-line" id="arrow-1" stroke="#3b82f6" stroke-width="3" marker-end="url(#arrowhead)" stroke-dasharray="5,5"/>
          <text x="270" y="140" class="flow-label" id="throughput-1">0/s</text>

          <!-- Queue → Processing -->
          <line x1="500" y1="150" x2="590" y2="150" class="flow-line" id="arrow-2" stroke="#3b82f6" stroke-width="3" marker-end="url(#arrowhead)" stroke-dasharray="5,5"/>
          <text x="540" y="140" class="flow-label" id="throughput-2">0/s</text>

          <!-- Processing → Storage -->
          <line x1="770" y1="150" x2="860" y2="150" class="flow-line" id="arrow-3" stroke="#3b82f6" stroke-width="3" marker-end="url(#arrowhead)" stroke-dasharray="5,5"/>
          <text x="810" y="140" class="flow-label" id="throughput-3">0/s</text>

          <!-- Processing → DLQ (vertical branch) -->
          <line x1="680" y1="220" x2="680" y2="280" class="flow-line dlq-arrow" id="arrow-dlq" stroke="#ef4444" stroke-width="3" marker-end="url(#arrowhead-error)" stroke-dasharray="5,5" opacity="0.3"/>
  ...
  <div class="modal-overlay" id="logModal">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button class="btn-close" onclick="closeLogModal()">&times;</button>
      </div>
  ...
  <div class="modal-overlay" id="debugHelpModal">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button class="btn-close" onclick="closeDebugHelpModal()">&times;</button>
      </div>
      <div class="modal-body">
        <h4 style="margin-top: 0;">Debug Flags Overview</h4>
        <p style="margin-bottom: 20px;">The Debug Control Panel allows you to test error pathways and system behavior by triggering specific debug flags through the API.</p>

        <div style="margin-bottom: 20px;">
          <h5 style="margin: 10px 0 8px 0;">⚠️ Validation Error</h5>
          <p style="margin: 0; color: var(--color-text-secondary); font-size: 14px;">Forces a validation error response (400 Bad Request). Use this to test client-side error handling for malformed requests.</p>
        ...
        <div style="margin-bottom: 20px;">
          ...
          <p style="margin: 0; color: var(--color-text-secondary); font-size: 14px;">Simulates processing failure after the event is queued. The event will be accepted but will fail during processing, triggering retry logic.</p>
        ...
        <div style="margin-bottom: 20px;">
          ...
          <p style="margin: 0; color: var(--color-text-secondary); font-size: 14px;">Injects artificial latency (default 2000ms) to simulate slow processing or network delays. Useful for performance testing.</p>
        ...
        <div style="margin-bottom: 20px;">
          ...
          <p style="margin: 0; color: var(--color-text-secondary); font-size: 14px;">Forces the event to be routed to the Dead Letter Queue after exhausting all retry attempts. Tests DLQ handling logic.</p>
        ...
        <ul style="margin: 0; padding-left: 20px; color: var(--color-text-secondary); font-size: 14px;">
          <li style="margin-bottom: 8px;"><strong>Mock Payloads:</strong> Choose from pre-built event templates or create custom JSON payloads</li>
          <li style="margin-bottom: 8px;"><strong>Quick Templates:</strong> One-click submission of common Zapier event types</li>
          <li style="margin-bottom: 8px;"><strong>Batch Testing:</strong> Execute all 4 debug flags sequentially with automatic progress tracking</li>
          <li style="margin-bottom: 8px;"><strong>Debug History:</strong> View your last 10 debug submissions with expandable response details</li>
          <li style="margin-bottom: 8px;"><strong>Pre-configured Scenarios:</strong> Run complex test scenarios with multiple debug flags</li>
          <li style="margin-bottom: 8px;"><strong>Keyboard Shortcuts:</strong> Quick access via Alt+D (toggle) and Alt+1-4 (trigger flags)</li>
        </ul>

        <h4 style="margin: 24px 0 12px 0;">Common Troubleshooting</h4>
        <ul style="margin: 0; padding-left: 20px; color: var(--color-text-secondary); font-size: 14px;">
          <li style="margin-bottom: 8px;"><strong>401 Unauthorized:</strong> Ensure your Bearer token is valid in the Authorization Token field</li>
          <li style="margin-bottom: 8px;"><strong>No response:</strong> Check browser console for network errors or CORS issues</li>
          <li style="margin-bottom: 8px;"><strong>Debug flags not working:</strong> Verify your API worker supports debug query parameters</li>
        </ul>

        <p style="margin-top: 24px; padding: 12px; background: var(--color-bg-tertiary); border-radius: 6px; font-size: 13px; color: var(--color-text-secondary);">
          <strong>Note:</strong> Debug flags are intended for development and demonstration purposes. Do not use in production environments.
        </p>
      </div>
    </div>
  </div>
  ...
  <div class="modal-overlay" id="batchGeneratorModal" style="display: none;">
    <div class="modal-content" style="max-width: 500px;">
      <div class="modal-header">
        ...
        <button class="btn-close" onclick="closeBatchGeneratorModal()">&times;</button>
      </div>
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <input type="number" id="batch-count-input" min="1" max="100" value="10" style="width: 100%; padding: 8px; border: 1px solid var(--color-border-secondary); border-radius: 4px;">
        </div>

        <div style="display: flex; gap: 12px; margin-top: 24px;">
          <button class="btn-submit" onclick="submitBatchGeneration()" style="flex: 1;">
            Generate Events
          </button>
          <button class="btn-secondary" onclick="closeBatchGeneratorModal()" style="flex: 1;">
            Cancel
          </button>
        </div>
        ...
        <div id="batch-progress-container" style="display: none; margin-top: 16px;">
          ...
          <p id="batch-progress-text" style="font-size: 12px; color: var(--color-text-secondary); margin-top: 8px; text-align: center;">0 / 0 events created</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal-overlay" id="templateBrowserModal" style="display: none;">
    <div class="modal-content" style="max-width: 800px; max-height: 80vh; overflow-y: auto;">
      <div class="modal-header">
        ...
        <button class="btn-close" onclick="closeTemplateBrowserModal()">&times;</button>
      </div>
      <div class="modal-body">
        <p style="font-size: 13px; color: var(--color-text-secondary); margin-bottom: 16px;">
        ...
        <!-- Templates Grid -->
        <div id="templates-browser-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px;"></div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    // BUG FIX 3: Removed response panel references - using toasts instead
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Validate inputs
      if (!bearerToken.trim()) {
        toast.error('Authorization token is required', 'VALIDATION_ERROR');
        return;
      }
      ...
      try {
        ...
        toast.error('Payload must be valid JSON', 'INVALID_JSON', {
          details: e.message
        });
        return;
      }
      ...
      if (metadata.trim()) {
        try {
          ...
          toast.error('Metadata must be valid JSON', 'INVALID_JSON', {
            details: e.message
          });
          return;
        }
      ...
      try {
        ...
        // BUG FIX 3: Use toasts instead of stale response panel
        if (res.ok) {
          toast.success('Event submitted successfully', {
            details: 'Event ID: ' + (data.data?.event_id || 'N/A'),
            action: {
              label: 'View in Inbox',
              callback: () => {
        ...
        toast.error('Failed to send request', 'NETWORK_ERROR', {
    ...
    // BUG FIX 2: Clear Form - Reset all form fields when reset button clicked
    ...
    class LoadingStateManager {
      ...
      showSkeleton(container, count = 5, columns = 3) {
        ...
        for (let i = 0; i < count; i++) {
          const widths = columns === 3 ? ['20%', '50%', '20%'] :
                         columns === 2 ? ['40%', '50%'] : ['100%'];

          const boxes = widths.map(w =>
            \`<div class="skeleton-box" style="width: \${w}; height: 20px;"></div>\`
          ).join('');
      ...
      showErrorSummary() {
        ...
        summary.innerHTML = \`
          ...
          <ul style="margin: 8px 0 0 20px;">
            \${Array.from(this.errors.values()).map(err => \`<li>\${err}</li>\`).join('')}
          </ul>
        \`;
    ...
    function renderLogs() {
      ...
      logsState.logs.forEach((log, index) => {
        const rowClass = log.log_level === 'error' ? 'log-row-error' :
                         log.log_level === 'warn' ? 'log-row-warn' : '';
    ...
    function updateLogsLastUpdatedText() {
      ...
      const now = new Date();
      document.getElementById('logsLastUpdatedText').textContent =
        'Auto-refresh every 3s • Updated: ' + now.toLocaleTimeString();
    }
    ...
    class DebugPanel {
      ...
      setupEventListeners() {
        ...
        // BUG FIX 5: Mock payload toggle - Initialize state and handle changes
        const mockToggle = document.getElementById('mock-payload-toggle');
        if (mockToggle) {
          // Initialize the state based on the checkbox default (checked)
          this.togglePayloadOptions(mockToggle.checked);

          mockToggle.addEventListener('change', (e) => {
            this.togglePayloadOptions(e.target.checked);
            // Show toast feedback
            if (e.target.checked) {
              toast.info('Mock payload enabled - using template selector');
            } else {
              toast.info('Mock payload disabled - using minimal payload');
            }
          });
        }
      ...
      async handleDebugFlag(event) {
        ...
        try {
          ...
          // BUG FIX 1: Show correct delay value for queue_delay instead of HTTP latency
          let detailsText = \`Latency: \${latency}ms\`;
          if (debugFlag === 'queue_delay') {
            const delay = document.getElementById('delay-input')?.value || 2000;
            detailsText = \`Queue Delay: \${delay}ms (HTTP: \${latency}ms)\`;
          }

          // BUG FIX 4: Update loading toast instead of removing and creating new one (avoid toast spam)
          if (response.ok) {
            loadingState.showSuccessAnimation();
            // Update the loading toast to success
            toast.update(loadingToastId, \`\${debugFlag}: \${response.status} \${response.statusText}\`, 'success');

            // Remove after showing success briefly
            setTimeout(() => {
              toast.remove(loadingToastId);
              // Show final toast with action button
              toast.success(
                \`\${debugFlag} completed\`,
                {
                  details: detailsText,
            ...
            toast.update(loadingToastId, \`\${debugFlag}: \${response.status} \${response.statusText}\`, 'error');
            setTimeout(() => toast.remove(loadingToastId), 4000);
          }
      ...
      renderHistory() {
        ...
        // BUG FIX 6: Properly format and escape JSON in debug history
        historyDiv.innerHTML = this.history.map((entry, index) => {
          const formattedJson = this.formatJson(entry.response);
          return \`
      ...
      formatJson(obj) {
        // Format JSON with proper indentation and escape HTML entities
      ...
      async runBatchTest() {
        ...
        if (errorCount === 0) {
          ...
          toast.success(
            ...
            { details: \`All \${flags.length} debug flags executed\` }
          );
        } else {
          toast.warning(
    ...
    class MockDataGenerator {
      ...
      randomEmailSubject() {
        const subjects = ['Order Confirmation', 'Payment Receipt', 'Welcome to TriggersAPI', 'Account Updated', 'Password Reset', 'Subscription Renewal'];
        return subjects[Math.floor(Math.random() * subjects.length)];
      }

      randomSMSMessage() {
        const messages = ['Your verification code is 123456', 'Your order has shipped', 'Payment received', 'Reminder: appointment today'];
      ...
      randomTaskTitle() {
        const tasks = ['Review pull request', 'Fix bug in authentication', 'Update documentation', 'Deploy to production', 'Run performance tests'];
    ...
    class FlowVisualization {
      ...
      createParticle() {
        ...
        const color = this.metrics.success_rate > 95 ? '#10b981' :
                     this.metrics.success_rate > 90 ? '#f59e0b' : '#ef4444';
...
export async function handleDashboard(request: Request): Promise<Response> {
	return new Response(dashboardHTML, {
		...
		headers: {
			...
			'Cache-Control': 'no-cache', // Changed from public, max-age=3600 to no-cache for development
		},
	});
}
