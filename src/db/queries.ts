...
export class EventQueries {
	...
	async getEventsByStatus(status: 'pending' | 'delivered' | 'failed'): Promise<Event[]> {
		const result = await this.db.prepare('SELECT * FROM events WHERE status = ? ORDER BY created_at DESC').bind(status).all<any>();
	 ...
	 * @returns Promise<Event[]> - Array of events
	 */
	async getEventsByStatusAndTimeRange(status: string, from: string, to: string, limit: number = 50, offset: number = 0): Promise<Event[]> {
		const result = await this.db
			.prepare(
				`
        SELECT * FROM events
	...
	async getTotalByStatus(status: string): Promise<number> {
		const result = await this.db.prepare('SELECT COUNT(*) as count FROM events WHERE status = ?').bind(status).first<{ count: number }>();
	 ...
	 */
	async updateEventStatus(eventId: string, newStatus: 'pending' | 'delivered' | 'failed' | 'retrying'): Promise<void> {
		const now = new Date().toISOString();

		await this.db.prepare('UPDATE events SET status = ?, updated_at = ? WHERE event_id = ?').bind(newStatus, now, eventId).run();
	}
	...
	async getEventById(eventId: string): Promise<Event | null> {
		const result = await this.db.prepare('SELECT * FROM events WHERE event_id = ?').bind(eventId).first<any>();
	...
	async deleteEvent(eventId: string): Promise<void> {
		await this.db.prepare('DELETE FROM events WHERE event_id = ?').bind(eventId).run();
	}
	 ...
	 */
	async countEventsByFilters(filters: { status?: 'pending' | 'delivered' | 'failed'; from?: string; to?: string }): Promise<number> {
	...
	async getEventsByAdvancedFilters(filters: AdvancedFilters): Promise<Event[]> {
		...
		if (cursor) {
			const cursorClause =
				order === 'desc' ? 'created_at < ? OR (created_at = ? AND event_id < ?)' : 'created_at > ? OR (created_at = ? AND event_id > ?)';
	...
	async countEventsByAdvancedFilters(filters: Omit<AdvancedFilters, 'limit' | 'offset' | 'cursor' | 'sort' | 'order'>): Promise<number> {
		const { status, from, to, createdDate, minRetries, maxRetries, metadata, payload } = filters;
